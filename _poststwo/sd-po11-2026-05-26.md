---
title: "11 Async Processing vs Sync Bottlenecks"
level: interview
layout: post
categories:
- sd-po Performance & Optimization
feature_text: |
  ## Performance & Optimization - 11 Async Processing vs Sync Bottlenecks
   Post by ailswan May. 26, 2026
feature_image: "https://picsum.photos/2560/600?image=611"
---
...
When discussing **Async Processing vs Sync Bottlenecks**, I frame it as a performance problem with clear measurement, bottleneck isolation, and trade-off management.

1. sync work blocks the user request path
2. async work improves perceived latency and resilience
3. only move non-critical or eventually consistent work async
4. define delivery guarantees and retry behavior
5. ensure idempotency for async handlers
6. track queue lag and failure rates
7. provide status or reconciliation when needed
8. avoid turning correctness-critical work into hidden async risk
...
Synchronous bottlenecks increase user latency and couple availability to every dependency. Async processing removes work from the critical path but introduces eventual consistency and operational complexity.
...
User request
  ↓
Synchronous validation and core write
  ↓
Outbox or queue
  ↓
Async workers
  ↓
Email, analytics, search index, notification, enrichment
```
...
* time to completion
* user-visible consistency delay
...
* use queues for notifications and analytics
* use outbox pattern for reliable event publishing
* make consumers idempotent
* set retry policy and DLQ
* return accepted status for long-running work
* separate critical state mutation from side effects
* monitor lag
...
* moving required validation async
* losing events between DB commit and queue publish
* not handling duplicate messages
* letting async queues become invisible bottlenecks
* not communicating eventual consistency to clients
...
> For **Async Processing vs Sync Bottlenecks**, I would start by defining the target metric and measuring the current system with traces, metrics, and profiling.
>
...
> I would approach **Async Processing vs Sync Bottlenecks** by measuring first, decomposing the critical path, and identifying the real bottleneck.
