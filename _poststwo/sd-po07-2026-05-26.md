---
title: "07 Read Replica vs Cache Optimization Strategy"
level: interview
layout: post
categories:
- sd-po Performance & Optimization
feature_text: |
  ## Performance & Optimization - 07 Read Replica vs Cache Optimization Strategy
   Post by ailswan May. 26, 2026
feature_image: "https://picsum.photos/2560/600?image=607"
---
...
When discussing **Read Replica vs Cache Optimization Strategy**, I frame it as a performance problem with clear measurement, bottleneck isolation, and trade-off management.

1. read replicas scale database reads while preserving query flexibility
2. caches reduce repeated reads with very low latency
3. replicas have replication lag; caches have staleness and invalidation
4. choose based on query pattern, freshness, and load shape
5. combine replicas and caches for hot read paths
6. keep writes on primary and route reads carefully
7. protect cache misses from stampedes
8. monitor lag, hit rate, and consistency impact
...
Both replicas and caches reduce primary database pressure, but they solve different problems and introduce different consistency trade-offs.
...
* use replicas for flexible SQL reads
* use cache for hot key-value reads
* apply TTL and versioned keys
* use cache-aside or write-through based on correctness needs
* route read-after-write requests to primary when necessary
* use request coalescing for hot misses
* monitor replica lag
* define stale-read tolerance explicitly
...
* using cache without invalidation ownership
* assuming replicas are strongly consistent
* sending all reads to replicas and breaking read-after-write flows
* cache stampede during expiration
...
> For **Read Replica vs Cache Optimization Strategy**, I would start by defining the target metric and measuring the current system with traces, metrics, and profiling.
>
...
> I would approach **Read Replica vs Cache Optimization Strategy** by measuring first, decomposing the critical path, and identifying the real bottleneck.
