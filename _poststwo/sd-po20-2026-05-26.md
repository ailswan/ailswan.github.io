---
title: "20 Latency Optimization in Distributed Systems"
level: interview
layout: post
categories:
- "Performance & Optimization"
feature_text: |
  ## Performance & Optimization - 20 Latency Optimization in Distributed Systems
...
1. start with critical-path decomposition
2. optimize p95 and p99, not only p50
3. separate queue time, service time, dependency time, and database time
4. remove or parallelize synchronous work
5. cache and precompute when correctness allows
6. use timeout budgets and deadline propagation
7. control overload with backpressure and load shedding
8. validate with tracing before and after changes
...
Latency optimization at staff level is about governing the full request path under real load. The hardest problems appear in the tail: overload, slow dependencies, retries, cache misses, locks, and cross-region communication.
...
* fan-out reduction
* parallel RPC where safe
* cache hot and stable reads
* precompute expensive read models
* bounded retries with jitter
* adaptive concurrency and load shedding
* avoid cross-region sync calls
...
* optimizing one service while dependency p99 dominates
* retry storms
* cache stampede
* over-parallelization
* ignoring queueing theory near saturation
* moving work async without lag monitoring
