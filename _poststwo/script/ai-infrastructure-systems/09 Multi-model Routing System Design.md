...
## Multi-model Routing System Design
...
Hello everyone.
Today I’d like to walk through **Multi-model Routing System Design**.
...
**(PPT 1: Title Slide — Multi-model Routing System Design)**
...
For **Multi-model Routing System Design**, I usually begin by asking three questions.
...
For **Multi-model Routing System Design**, the important trade-offs are usually:
...
For **Multi-model Routing System Design**, remember three things.
...
大家好。
今天我们来讲 **Multi-model Routing System Design**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> Multi-model routing 是在 quality、latency、cost、capacity 和 policy 之间做选择。Staff 级要讲 task classifier、tenant policy、fallback、shadow testing、A/B、regional capacity 和 quality monitoring。
...
讲 **Multi-model Routing System Design** 时，记住三个点。
