...
## Model Serving Infrastructure (Kubernetes + LLMs)
...
Hello everyone.
Today I’d like to walk through **Model Serving Infrastructure (Kubernetes + LLMs)**.
...
**(PPT 1: Title Slide — Model Serving Infrastructure (Kubernetes + LLMs))**
...
For **Model Serving Infrastructure (Kubernetes + LLMs)**, I usually begin by asking three questions.
...
For **Model Serving Infrastructure (Kubernetes + LLMs)**, the important trade-offs are usually:
...
For **Model Serving Infrastructure (Kubernetes + LLMs)**, remember three things.
...
大家好。
今天我们来讲 **Model Serving Infrastructure (Kubernetes + LLMs)**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> Kubernetes + LLM serving 要处理 GPU node pool、model loading、readiness、autoscaling、traffic routing、canary、streaming connection 和 GPU observability。Staff 级要指出默认 K8s scheduler 对 LLM/GPU 不够，需要额外调度和容量管理。
...
讲 **Model Serving Infrastructure (Kubernetes + LLMs)** 时，记住三个点。
