...
## Batch vs Real-time LLM Inference Systems
...
Hello everyone.
Today I’d like to walk through **Batch vs Real-time LLM Inference Systems**.
...
**(PPT 1: Title Slide — Batch vs Real-time LLM Inference Systems)**
...
For **Batch vs Real-time LLM Inference Systems**, I usually begin by asking three questions.
...
For **Batch vs Real-time LLM Inference Systems**, the important trade-offs are usually:
...
For **Batch vs Real-time LLM Inference Systems**, remember three things.
...
大家好。
今天我们来讲 **Batch vs Real-time LLM Inference Systems**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> Real-time LLM inference 追求低延迟，batch inference 追求吞吐和成本。Staff 级回答要说 online/batch pool 隔离、priority scheduling、micro-batching、checkpoint、preemption、SLA 和 cost trade-off。
...
讲 **Batch vs Real-time LLM Inference Systems** 时，记住三个点。
