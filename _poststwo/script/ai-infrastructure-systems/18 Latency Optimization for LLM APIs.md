...
## Latency Optimization for LLM APIs
...
Hello everyone.
Today I’d like to walk through **Latency Optimization for LLM APIs**.
...
**(PPT 1: Title Slide — Latency Optimization for LLM APIs)**
...
For **Latency Optimization for LLM APIs**, I usually begin by asking three questions.
...
For **Latency Optimization for LLM APIs**, the important trade-offs are usually:
...
For **Latency Optimization for LLM APIs**, remember three things.
...
大家好。
今天我们来讲 **Latency Optimization for LLM APIs**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> LLM API latency 优化要看 TTFT、TTL、queue time、prefill、decode、streaming、prompt size 和 RAG/tool latency。Staff 级要讲 prompt caching、continuous batching、fast lane、model routing、streaming 和 safety latency。
...
讲 **Latency Optimization for LLM APIs** 时，记住三个点。
