...
Hello everyone.
Today I’d like to walk through **AI Content Moderation Pipeline**.
...
**(PPT 1: Title Slide — AI Content Moderation Pipeline)**
...
For **AI Content Moderation Pipeline**, I usually begin by asking three questions.
...
For **AI Content Moderation Pipeline**, the important trade-offs are usually:
...
For **AI Content Moderation Pipeline**, remember three things.
...
大家好。
今天我们来讲 **AI Content Moderation Pipeline**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> AI content moderation pipeline 要处理 input/output、policy taxonomy、risk score、human review、appeal、audit 和 drift。Staff 级要强调阈值、误杀/漏放 trade-off、地区政策、review SLA 和 feedback loop。
...
讲 **AI Content Moderation Pipeline** 时，记住三个点。
