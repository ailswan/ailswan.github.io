...
## When to Choose Simple vs Complex Design
...
Hello everyone.
Today I’d like to walk through **When to Choose Simple vs Complex Design**.
...
**(PPT 1: Title Slide — When to Choose Simple vs Complex Design)**
...
For **When to Choose Simple vs Complex Design**, I usually begin by asking three questions.
...
* start simple when requirements are simple
* add complexity only for real constraints
...
For **When to Choose Simple vs Complex Design**, the important trade-offs are usually:

* prefer evolvable architecture
* avoid premature optimization
* know when scale requires complexity
...
> I would approach **When to Choose Simple vs Complex Design** with a clear interview structure: clarify, scope, estimate, design, deep dive, trade off, and summarize. The goal is to demonstrate senior engineering judgment under ambiguity. A strong answer should be easy to follow, grounded in requirements, explicit about trade-offs, and realistic about failure handling, observability, and operational cost.
...
For **When to Choose Simple vs Complex Design**, remember three things.
...
大家好。
今天我们来讲 **When to Choose Simple vs Complex Design**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> 简单设计不是低级，复杂设计也不是高级。Senior 级要知道什么时候 scale、consistency、availability、compliance 真的需要复杂度，并说明演进路径。
...
讲 **When to Choose Simple vs Complex Design** 时，记住三个点。
