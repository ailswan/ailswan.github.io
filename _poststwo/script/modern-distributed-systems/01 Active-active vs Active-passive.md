...
Hello everyone.
Today I’d like to walk through **Active-active vs Active-passive**.
...
**(PPT 1: Title Slide — Active-active vs Active-passive)**
...
For **Active-active vs Active-passive**, I usually begin by asking three questions.
...
1. Multi-region System Design Active-active vs Active-passive
2. Core Framework
3. Why Multi-region Systems Exist
4. Basic Idea
5. What Is Active-passive?
...
For **Active-active vs Active-passive**, the important trade-offs are usually:
...
> Multi-region system design is about surviving regional failures, reducing global latency, and meeting disaster recovery requirements. The main architectural choice is active-passive versus active-active. In active-passive, one primary region serves live traffic while another region stays on standby. If the primary region fails, traffic is failed over to the passive region, and the standby database may be promoted. This design is simpler, cheaper, and easier to reason about, especially when strong consistency and write correctness matter. The downside is that failover can take longer, and replication lag can cause data loss depending on the RP
...
For **Active-active vs Active-passive**, remember three things.
...
大家好。
今天我们来讲 **Active-active vs Active-passive**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> Active-active vs Active-passive 的讲解重点是先讲清楚问题背景，再拆架构、流程、瓶颈、trade-off 和 Staff 级总结。
...
第一，Multi-region System Design Active-active vs Active-passive。
第二，Core Framework。
第三，Why Multi-region Systems Exist。
第四，Basic Idea。
第五，What Is Active-passive?。
...
讲 **Active-active vs Active-passive** 时，记住三个点。
