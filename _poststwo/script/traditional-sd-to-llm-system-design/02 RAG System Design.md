...
Hello everyone.
Today I’d like to walk through **RAG System Design**.
...
**(PPT 1: Title Slide — RAG System Design)**
...
For this topic, the traditional version is a traditional search system with document indexing, query serving, filters, and relevance ranking.
...
For **RAG System Design**, the LLM-specific upgrade is chunking, embeddings, vector search, reranking, context packing, grounded generation, and citations.
...
User query arrives
  ↓
Retriever gets candidates
  ↓
ACL filter runs
  ↓
Reranker orders evidence
  ↓
Context builder packs tokens
  ↓
...
For **RAG System Design**, the important trade-offs are:

* recall versus latency
* freshness versus index cost
* chunk size versus answer quality
* citation quality versus token budget
...
For **RAG System Design**, I would monitor:
...
> For **RAG System Design**, I would start from the traditional system design baseline: API gateway, services, storage, cache, queues, scaling, and reliability.
...
这个 topic 的传统版本是：a traditional search system with document indexing, query serving, filters, and relevance ranking。
...
升级到 LLM 系统后，新增的是：chunking, embeddings, vector search, reranking, context packing, grounded generation, and citations。
...
> 对于 **RAG System Design**，我会先从传统系统设计 baseline 开始：gateway、service、storage、cache、queue、scaling 和 reliability。
