...
Hello everyone.
Today I’d like to walk through **Vector DB Scaling**.
...
**(PPT 1: Title Slide — Vector DB Scaling)**
...
For this topic, the traditional version is traditional database scaling covers indexing, partitioning, replication, caching, compaction, and hot keys.
...
For **Vector DB Scaling**, the LLM-specific upgrade is ANN indexes, recall-latency trade-offs, metadata filtering, hybrid search, embedding versioning, upsert/delete semantics, and memory-heavy serving.
...
Documents embedded
  ↓
Vectors and metadata written
  ↓
Shard manager assigns partition
  ↓
ANN index builds
  ↓
Query embedding arrives
  ↓
Metadata filters planned
  ↓
ANN search retrieves candidates
  ↓
Hybrid rerank runs
  ↓
Top-K returned
...
For **Vector DB Scaling**, the important trade-offs are:

* recall versus latency
* memory cost versus speed
* freshness versus rebuild cost
* metadata filtering versus ANN efficiency
...
For **Vector DB Scaling**, I would monitor:
...
> For **Vector DB Scaling**, I would start from the traditional system design baseline: API gateway, services, storage, cache, queues, scaling, and reliability.
...
这个 topic 的传统版本是：traditional database scaling covers indexing, partitioning, replication, caching, compaction, and hot keys。
...
升级到 LLM 系统后，新增的是：ANN indexes, recall-latency trade-offs, metadata filtering, hybrid search, embedding versioning, upsert/delete semantics, and memory-heavy serving。
...
> 对于 **Vector DB Scaling**，我会先从传统系统设计 baseline 开始：gateway、service、storage、cache、queue、scaling 和 reliability。
