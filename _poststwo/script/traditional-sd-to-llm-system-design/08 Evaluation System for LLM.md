...
Hello everyone.
Today I’d like to walk through **Evaluation System for LLM**.
...
**(PPT 1: Title Slide — Evaluation System for LLM)**
...
For this topic, the traditional version is a traditional evaluation system uses unit tests, integration tests, SLOs, dashboards, and production metrics.
...
For **Evaluation System for LLM**, the LLM-specific upgrade is golden datasets, rubric grading, pairwise comparison, model-graded evals, human labels, prompt regression, and online feedback.
...
Production traces sampled
  ↓
Examples cleaned and labeled
  ↓
Rubrics versioned
  ↓
Candidate prompt runs
  ↓
Model grader scores output
  ↓
Human labels review sample
  ↓
Regression gate compares baseline
  ↓
Canary rollout monitored
```
...
For **Evaluation System for LLM**, the important trade-offs are:

* offline eval versus online reality
* model grader speed versus bias
* coverage versus labeling cost
* strict gate versus iteration speed
...
For **Evaluation System for LLM**, I would monitor:

* task success rate
* win rate versus baseline
* rubric score
* label agreement
* regression count
...
> For **Evaluation System for LLM**, I would start from the traditional system design baseline: API gateway, services, storage, cache, queues, scaling, and reliability.
...
大家好。
今天我们讲 **Evaluation System for LLM**。
...
这个 topic 的传统版本是：a traditional evaluation system uses unit tests, integration tests, SLOs, dashboards, and production metrics。
...
升级到 LLM 系统后，新增的是：golden datasets, rubric grading, pairwise comparison, model-graded evals, human labels, prompt regression, and online feedback。
...
> 对于 **Evaluation System for LLM**，我会先从传统系统设计 baseline 开始：gateway、service、storage、cache、queue、scaling 和 reliability。
