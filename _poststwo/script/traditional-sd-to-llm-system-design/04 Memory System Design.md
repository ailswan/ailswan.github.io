...
Hello everyone.
Today I’d like to walk through **Memory System Design**.
...
**(PPT 1: Title Slide — Memory System Design)**
...
For this topic, the traditional version is a traditional profile and history system with user data, preferences, sessions, access control, and retention.
...
For **Memory System Design**, the LLM-specific upgrade is short-term context, long-term extracted memory, semantic retrieval, recency and importance scoring, and deletion governance.
...
Conversation happens
  ↓
Memory extractor finds durable fact
  ↓
Classifier assigns memory type
  ↓
Scope attached
  ↓
Memory stored and embedded
  ↓
Future request arrives
  ↓
Memory retrieved
  ↓
Freshness and privacy filter runs
  ↓
Selected memory enters context
```
...
For **Memory System Design**, the important trade-offs are:

* personalization versus privacy
* saving more versus prompt pollution
* recency versus importance
* memory recall versus precision
...
For **Memory System Design**, I would monitor:
...
> For **Memory System Design**, I would start from the traditional system design baseline: API gateway, services, storage, cache, queues, scaling, and reliability.
...
这个 topic 的传统版本是：a traditional profile and history system with user data, preferences, sessions, access control, and retention。
...
升级到 LLM 系统后，新增的是：short-term context, long-term extracted memory, semantic retrieval, recency and importance scoring, and deletion governance。
...
> 对于 **Memory System Design**，我会先从传统系统设计 baseline 开始：gateway、service、storage、cache、queue、scaling 和 reliability。
