...
Hello everyone.
Today I’d like to walk through **Cassandra vs DynamoDB**.
...
**(PPT 1: Title Slide — Cassandra vs DynamoDB)**
...
For **Cassandra vs DynamoDB**, I usually begin by asking three questions.
...
For **Cassandra vs DynamoDB**, the important trade-offs are usually:

* compare operational ownership
* compare scaling approach
...
For **Cassandra vs DynamoDB**, remember three things.
...
大家好。
今天我们来讲 **Cassandra vs DynamoDB**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> Cassandra vs DynamoDB 的核心差异是 self-managed control vs managed service。Staff 级要比较 data model、partition key、consistency、operations、scaling、cost 和团队能力。
...
讲 **Cassandra vs DynamoDB** 时，记住三个点。
