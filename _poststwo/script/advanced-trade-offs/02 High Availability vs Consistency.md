...
Hello everyone.
Today I’d like to walk through **High Availability vs Consistency**.
...
In this video, we will focus on decision criteria, correctness, availability, consistency, latency, cost, and operational complexity.

By the end, you should be able to explain this topic clearly in a senior or staff-level interview.

**(PPT 1: Title Slide — High Availability vs Consistency)**
...
For **High Availability vs Consistency**, I usually begin by asking three questions.
...
* define availability and consistency separately
* explain CAP trade-off under partition
* identify business correctness needs
...
For **High Availability vs Consistency**, the important trade-offs are usually:
...
For **High Availability vs Consistency**, remember three things.
...
大家好。
今天我们来讲 **High Availability vs Consistency**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> High Availability vs Consistency 不是二选一口号，而是按业务操作选择。读路径可能接受 stale data，支付、库存、ledger 等写路径通常需要更强一致性。
...
讲 **High Availability vs Consistency** 时，记住三个点。
