...
Hello everyone.
Today I’d like to walk through **Outbox Pattern**.
...
**(PPT 1: Title Slide — Outbox Pattern)**
...
For **Outbox Pattern**, I usually begin by asking three questions.
...
* store event with business transaction
...
For **Outbox Pattern**, the important trade-offs are usually:
...
For **Outbox Pattern**, remember three things.
...
大家好。
今天我们来讲 **Outbox Pattern**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> Outbox Pattern 解决 database update 和 message publish 的 dual-write 问题。核心是在同一个 DB transaction 写业务数据和 outbox event，再异步可靠发布。
