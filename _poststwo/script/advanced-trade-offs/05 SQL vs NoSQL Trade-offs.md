...
Hello everyone.
Today I’d like to walk through **SQL vs NoSQL Trade-offs**.
...
**(PPT 1: Title Slide — SQL vs NoSQL Trade-offs)**
...
For **SQL vs NoSQL Trade-offs**, I usually begin by asking three questions.
...
* define relational strengths
* define NoSQL strengths
* map choice to query patterns
...
For **SQL vs NoSQL Trade-offs**, the important trade-offs are usually:

* consider consistency and transactions
* consider schema evolution
* consider scale and sharding
...
For **SQL vs NoSQL Trade-offs**, remember three things.
...
大家好。
今天我们来讲 **SQL vs NoSQL Trade-offs**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> SQL 适合关系清晰、事务强、查询灵活的场景；NoSQL 适合访问模式明确、高吞吐、水平扩展的场景。Staff 级要按数据模型和 access pattern 选择。
...
讲 **SQL vs NoSQL Trade-offs** 时，记住三个点。
