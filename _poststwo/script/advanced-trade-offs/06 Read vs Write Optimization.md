...
Hello everyone.
Today I’d like to walk through **Read vs Write Optimization**.
...
**(PPT 1: Title Slide — Read vs Write Optimization)**
...
For **Read vs Write Optimization**, I usually begin by asking three questions.
...
* optimize reads with cache or replicas
...
For **Read vs Write Optimization**, the important trade-offs are usually:

* optimize writes with batching or logs
...
For **Read vs Write Optimization**, remember three things.
...
大家好。
今天我们来讲 **Read vs Write Optimization**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> 读优化常用 cache、replica、index、denormalized read model；写优化关注 append、batch、queue、减少 index 和 write amplification。Staff 级要按 read/write ratio 和一致性要求选择。
...
讲 **Read vs Write Optimization** 时，记住三个点。
