...
## Optimizing API Gateway Latency
...
Hello everyone.
Today I’d like to walk through **Optimizing API Gateway Latency**.
...
**(PPT 1: Title Slide — Optimizing API Gateway Latency)**
...
For **Optimizing API Gateway Latency**, I usually begin by asking three questions.
...
* gateway sits on every request, so small overhead matters
* separate routing, auth, rate limiting, transformation, and observability cost
* cache stable auth or config decisions carefully
...
For **Optimizing API Gateway Latency**, the important trade-offs are usually:

* avoid heavy business logic in gateway
* keep filters predictable and bounded
* use connection reuse and efficient TLS termination
...
For **Optimizing API Gateway Latency**, remember three things.
...
大家好。
今天我们来讲 **Optimizing API Gateway Latency**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> API Gateway 在所有请求路径上，所以每一点开销都会被放大。优化重点是减少插件和远程校验、缓存稳定的 auth/policy/routing 信息、连接复用、避免 heavy transformation，并分别观测 gateway latency 和 backend latency。
...
讲 **Optimizing API Gateway Latency** 时，记住三个点。
