...
Hello everyone.
Today I’d like to walk through **High-throughput Logging Systems**.
...
**(PPT 1: Title Slide — High-throughput Logging Systems)**
...
For **High-throughput Logging Systems**, I usually begin by asking three questions.
...
* local buffering absorbs bursts
...
For **High-throughput Logging Systems**, the important trade-offs are usually:

* sampling controls cost and volume
* schema and cardinality matter
* backpressure and dropping policy must be explicit
...
For **High-throughput Logging Systems**, remember three things.
...
大家好。
今天我们来讲 **High-throughput Logging Systems**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> 高吞吐日志系统不能阻塞主链路。核心是 async logging、bounded buffer、batch、compression、sampling、structured log、cardinality 控制和明确的丢弃策略。audit log 和 debug log 要分开设计。
...
讲 **High-throughput Logging Systems** 时，记住三个点。
