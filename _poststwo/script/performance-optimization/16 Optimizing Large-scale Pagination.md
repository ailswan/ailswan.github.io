...
## Optimizing Large-scale Pagination
...
Hello everyone.
Today I’d like to walk through **Optimizing Large-scale Pagination**.
...
**(PPT 1: Title Slide — Optimizing Large-scale Pagination)**
...
For **Optimizing Large-scale Pagination**, I usually begin by asking three questions.
...
* offset pagination is simple but slow at large offsets
* cursor/keyset pagination is better for deep scrolling
...
For **Optimizing Large-scale Pagination**, the important trade-offs are usually:

* pagination must handle inserts and deletes
* opaque cursors hide implementation details
* search systems may use search-after
...
For **Optimizing Large-scale Pagination**, remember three things.
...
大家好。
今天我们来讲 **Optimizing Large-scale Pagination**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> 大规模分页不能只用 offset。深页会扫描并丢弃大量行，延迟越来越高。Staff 级要用 cursor/keyset pagination、稳定排序、复合索引、opaque cursor、page-size limit，并说明一致性和插入删除时的行为。
...
讲 **Optimizing Large-scale Pagination** 时，记住三个点。
