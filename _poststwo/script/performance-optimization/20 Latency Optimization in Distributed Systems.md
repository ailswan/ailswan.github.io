...
* start with critical-path decomposition
* optimize p95 and p99, not only p50
* separate queue time, service time, dependency time, and database time
...
* remove or parallelize synchronous work
* cache and precompute when correctness allows
* use timeout budgets and deadline propagation
...
> Latency optimization 要拆 critical path，看 p95/p99，减少同步依赖，控制 fan-out、queueing、retry 和 cross-region call。Staff 级重点是端到端治理，而不是单点函数优化。
