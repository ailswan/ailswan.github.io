...
## Async Processing vs Sync Bottlenecks
...
Hello everyone.
Today I’d like to walk through **Async Processing vs Sync Bottlenecks**.
...
**(PPT 1: Title Slide — Async Processing vs Sync Bottlenecks)**
...
For **Async Processing vs Sync Bottlenecks**, I usually begin by asking three questions.
...
* sync work blocks the user request path
* async work improves perceived latency and resilience
* only move non-critical or eventually consistent work async
...
For **Async Processing vs Sync Bottlenecks**, the important trade-offs are usually:

* define delivery guarantees and retry behavior
* ensure idempotency for async handlers
* track queue lag and failure rates
...
For **Async Processing vs Sync Bottlenecks**, remember three things.
...
大家好。
今天我们来讲 **Async Processing vs Sync Bottlenecks**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> Async 可以把非关键工作移出用户请求路径，降低延迟和耦合，但会带来 eventual consistency、retry、幂等和可观测性问题。Staff 级要强调 outbox、DLQ、queue lag、reconciliation 和边界：核心状态变更仍要可靠。
...
讲 **Async Processing vs Sync Bottlenecks** 时，记住三个点。
