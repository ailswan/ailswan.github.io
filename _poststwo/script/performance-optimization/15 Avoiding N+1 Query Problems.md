...
Hello everyone.
Today I’d like to walk through **Avoiding N+1 Query Problems**.
...
**(PPT 1: Title Slide — Avoiding N+1 Query Problems)**
...
For **Avoiding N+1 Query Problems**, I usually begin by asking three questions.
...
* N+1 means one list query plus one query per item
* query count grows with result size
* ORM lazy loading often hides the issue
...
For **Avoiding N+1 Query Problems**, the important trade-offs are usually:

* fix with eager loading, joins, IN queries, or DataLoader
* control over-fetch and duplicate rows
* consider read models for hot endpoints
...
For **Avoiding N+1 Query Problems**, remember three things.
...
大家好。
今天我们来讲 **Avoiding N+1 Query Problems**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> N+1 是先查列表，再为每个元素单独查关联对象，导致 query count 随结果数量线性增长。Staff 级回答要说 trace query count，用 eager loading、join、WHERE IN、DataLoader、read model 控制数据库 round trip 上限。
...
讲 **Avoiding N+1 Query Problems** 时，记住三个点。
