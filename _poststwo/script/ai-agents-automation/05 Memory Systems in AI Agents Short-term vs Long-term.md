...
Hello everyone.
Today I’d like to walk through **Memory Systems in AI Agents Short-term vs Long-term**.
...
**(PPT 1: Title Slide — Memory Systems in AI Agents Short-term vs Long-term)**
...
For **Memory Systems in AI Agents Short-term vs Long-term**, I usually begin by asking three questions.
...
For **Memory Systems in AI Agents Short-term vs Long-term**, the important trade-offs are usually:
...
> Memory is critical for AI agents because agents often perform multi-step tasks across conversations, tools, and sessions. I usually divide memory into short-term memory, working memory, long-term memory, and retrieval-based memory. Short-term memory is the current conversation context. It helps the agent understand references like “continue,” “that result,” or “summarize the previous answer.” But short-term memory is limited by the model’s context window, so the system must summarize, rank, retrieve, or discard context when the conversation becomes large. Working memory is the active task state. It tracks the current goal, completed steps, pe
...
For **Memory Systems in AI Agents Short-term vs Long-term**, remember three things.
...
大家好。
今天我们来讲 **Memory Systems in AI Agents Short-term vs Long-term**。

这个 topic 的重点不是背一个固定答案，而是学会用 Staff 级思维解释一个系统设计问题。

一句话总结：

> Memory Systems in AI Agents Short-term vs Long-term 的讲解重点是先讲清楚问题背景，再拆架构、流程、瓶颈、trade-off 和 Staff 级总结。
...
第一，Core Framework。
第二，Why Do AI Agents Need Memory?。
第三，Basic Flow。
第四，Types of Memory。
第五，Main Types。
...
讲 **Memory Systems in AI Agents Short-term vs Long-term** 时，记住三个点。
