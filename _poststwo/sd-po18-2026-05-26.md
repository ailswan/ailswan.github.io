---
title: "18 Optimizing Serialization Formats (JSON vs Protobuf)"
level: interview
layout: post
categories:
- sd-po Performance & Optimization
feature_text: |
  ## Performance & Optimization - 18 Optimizing Serialization Formats (JSON vs Protobuf)
   Post by ailswan May. 26, 2026
feature_image: "https://picsum.photos/2560/600?image=618"
---
...
When discussing **Optimizing Serialization Formats (JSON vs Protobuf)**, I frame it as a performance problem with clear measurement, bottleneck isolation, and trade-off management.

1. serialization affects payload size, CPU, compatibility, and debugging
2. JSON is simple and human-readable
3. Protobuf is compact, typed, and efficient
4. public APIs often favor JSON
5. internal high-throughput RPC often favors Protobuf
6. schema evolution matters more than raw speed
7. measure encode/decode cost and payload size
8. choose format per boundary, not globally
...
Serialization sits on every service boundary. The wrong format can waste CPU and bandwidth or create schema evolution and debugging problems.
...
* use JSON for public compatibility
* use Protobuf/gRPC for internal high-QPS RPC
* define backward-compatible schema changes
* avoid oversized nested payloads
* compress large responses when useful
* generate clients from schemas
* monitor parse errors
* keep observability for binary payloads
...
* choosing Protobuf only for speed while ignoring schema governance
* using JSON with huge unbounded payloads
* breaking backward compatibility
* not preserving unknown fields where needed
* losing debuggability for incidents
...
> For **Optimizing Serialization Formats (JSON vs Protobuf)**, I would start by defining the target metric and measuring the current system with traces, metrics, and profiling.
>
...
> I would approach **Optimizing Serialization Formats (JSON vs Protobuf)** by measuring first, decomposing the critical path, and identifying the real bottleneck.
