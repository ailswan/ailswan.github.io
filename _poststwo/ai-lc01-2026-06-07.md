...
note: e
feature_text: |
  ## AI LeetCode Patterns - 01 RAG Top-K Retrieval
...
> For RAG Top-K Retrieval, I would first translate the AI behavior into a concrete algorithmic problem.
> The baseline is candidate generation, scoring, Top-K selection, reranking, and context packing.
...
> For this topic, the deterministic part is candidate generation, scoring, Top-K selection, reranking, and context packing.
...
> I would map RAG Top-K Retrieval to a LeetCode pattern by identifying the state, ordering rule, and constraint.
...
> I would explain RAG Top-K Retrieval as an AI system built on top of a concrete LeetCode-style algorithm.
> The algorithmic core is candidate generation, scoring, Top-K selection, reranking, and context packing.
...
> RAG Top-K Retrieval 不是单纯的 prompt 技巧，而是一个可以映射到 LeetCode pattern 的系统问题。
> 它的核心是 candidate generation, scoring, Top-K selection, reranking, and context packing。
...
> 我会把 RAG Top-K Retrieval 先抽象成算法题。
...
> 对于 RAG Top-K Retrieval，我不会只说调用 LLM。
...
> RAG Top-K Retrieval 的 Staff 级回答，要从 LeetCode pattern 讲到 production AI system。
> 算法部分解决局部正确性：candidate generation, scoring, Top-K selection, reranking, and context packing。
