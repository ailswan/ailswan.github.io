---
title: 05 Tool Calling Architecture
level: interview
layout: post
categories:
...
note: e
feature_text: |
  ## From Traditional SD to LLM System Design - 05 Tool Calling Architecture
   Post by ailswan June. 07, 2026
feature_image: "https://picsum.photos/2560/600?image=974"
---
...
This topic is about upgrading a **traditional system design** answer into an **LLM system design** answer for **Tool Calling Architecture**.

**Traditional SD baseline:**

A traditional API integration layer validates requests, calls downstream services, retries failures, and returns structured results.

**LLM system upgrade:**

Tool calling architecture lets the model request external actions, but the system must validate schema, authorize execution, sandbox side effects, parse observations, and audit every call.
...
> For Tool Calling Architecture, I would start from the traditional system design baseline, then explain what changes when the core capability is powered by an LLM.
...
Model proposes tool call
  ↓
Router maps to registered tool
  ↓
...
Executor calls tool
  ↓
Observation parser normalizes result
  ↓
...
> For Tool Calling Architecture, I would design a layered architecture.
...
For **Tool Calling Architecture**, I would specifically discuss:
...
A Staff-level tool-calling design treats tools as an execution boundary, not a model capability. The platform must control schemas, permissions, side effects, retries, approvals, and audit logs.

**Memorize this answer:**

> For Tool Calling Architecture, I would not replace traditional system design with an LLM call.
...
A: A Staff-level tool-calling design treats tools as an execution boundary, not a model capability. The platform must control schemas, permissions, side effects, retries, approvals, and audit logs.
...
>For Tool Calling Architecture, I would start with the traditional system design baseline and then upgrade it with LLM-specific components. The traditional baseline gives us API gateway, auth, stateless services, storage, cache, queues, scaling, and reliability. The LLM upgrade adds prompt/context construction, model routing, retrieval or memory, tool execution, safety checks, evaluation, token cost, and model observability.

### Memorization Paragraph 2

>The key design principle for Tool Calling Architecture is that the model should not be treated as the whole system. The model is one component behind an orchestration layer. The orchestration layer owns context, permissions, model choice, tool access, validation, cost control, and traces.

### Memorization Paragraph 3

>Compared with traditional SD, Tool Calling Architecture introduces probabilistic quality. That means I need both system metrics and quality metrics: latency, error rate, and cost on one side; groundedness, safety, user correction rate, and task success on the other side.
...
讨论 **Tool Calling Architecture** 时，我会按这个结构回答：
...
> 对于 Tool Calling Architecture，我会先从传统系统设计开始，而不是直接说调用 LLM。
...
在 **Tool Calling Architecture** 中具体要讲：
...
> 讲 Tool Calling Architecture 时，我会明确区分传统系统部分和 LLM 特有部分。
...
>Tool Calling Architecture 的回答不能停留在传统 SD，也不能只说调用 LLM。我的回答方式是先建立传统系统设计 baseline，然后增加 LLM orchestration layer。传统部分保证服务稳定，LLM 部分负责智能能力，系统边界负责安全、成本、评估和可观测性。

### 背诵段落 2

>传统系统设计主要关注 API、服务、数据库、缓存、队列和扩展。升级到 Tool Calling Architecture 后，还要关注 prompt version、model version、context window、retrieval、memory、tool calling、safety policy、eval pipeline 和 token cost。
...
>我会把 Tool Calling Architecture 拆成 online path 和 offline path。online path 负责用户请求、latency、safety 和 fallback；offline path 负责 index、eval、prompt test、cost analysis、quality regression 和 rollout。

### 背诵段落 5

>如果 Tool Calling Architecture 出现质量问题，我会通过 trace debug：用户输入是什么、context 是什么、prompt/model 版本是什么、retrieval 和 tool 结果是什么、safety decision 是什么、最终输出为什么通过。

---

## 19️⃣ 中文 Staff 总结

> Tool Calling Architecture 的 Staff 级答案，是从传统 SD 过渡到 LLM 系统设计。
> 传统 SD 提供稳定的分布式系统基础。
...
Tool Calling Architecture = Traditional SD baseline + LLM orchestration layer + safety/eval/cost/observability.
