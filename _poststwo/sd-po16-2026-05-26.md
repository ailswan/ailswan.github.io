---
title: "16 Optimizing Large-scale Pagination"
level: interview
layout: post
categories:
- sd-po Performance & Optimization
feature_text: |
  ## Performance & Optimization - 16 Optimizing Large-scale Pagination
   Post by ailswan May. 26, 2026
feature_image: "https://picsum.photos/2560/600?image=616"
---
...
When discussing **Optimizing Large-scale Pagination**, I frame it as a performance problem with clear measurement, bottleneck isolation, and trade-off management.

1. offset pagination is simple but slow at large offsets
2. cursor/keyset pagination is better for deep scrolling
3. stable ordering is required
4. pagination must handle inserts and deletes
5. opaque cursors hide implementation details
6. search systems may use search-after
7. define consistency and freshness expectations
8. protect endpoints with page-size limits
...
Large-scale pagination becomes expensive when the database must scan and discard many rows or when ordering changes while users page through results.
...
* query latency by page depth
* rows scanned vs returned
* page size
* cursor error rate
* index usage
* duplicate or missing item rate
* database CPU
* p99 for deep pages
...
* use composite indexes matching sort order
* use opaque signed cursors
* limit page size
* avoid COUNT(*) on huge tables when not needed
* use search-after for search indexes
* snapshot when strict consistency is required
* include tie-breaker ID in ordering
...
* missing tie-breaker in cursor order
* exposing raw internal IDs without need
* promising strict consistency without snapshot support
...
> For **Optimizing Large-scale Pagination**, I would start by defining the target metric and measuring the current system with traces, metrics, and profiling.
>
...
> I would approach **Optimizing Large-scale Pagination** by measuring first, decomposing the critical path, and identifying the real bottleneck.
