---
title: "08 Compression vs Compute Trade-offs"
level: interview
layout: post
categories:
- sd-po Performance & Optimization
feature_text: |
  ## Performance & Optimization - 08 Compression vs Compute Trade-offs
   Post by ailswan May. 26, 2026
feature_image: "https://picsum.photos/2560/600?image=608"
---
...
When discussing **Compression vs Compute Trade-offs**, I frame it as a performance problem with clear measurement, bottleneck isolation, and trade-off management.

1. compression saves bandwidth and storage
2. compression costs CPU and sometimes latency
3. benefit depends on payload size, entropy, and network speed
4. choose algorithm by ratio, speed, and compatibility
5. compress large text responses and logs more often than tiny payloads
6. avoid compressing already-compressed data
7. measure end-to-end latency, not only size reduction
8. consider client CPU and battery cost
...
Compression can improve throughput and latency when network or storage is the bottleneck, but it can hurt performance when CPU is the bottleneck or payloads are small.
...
* set size thresholds before compression
* use faster algorithms for latency-sensitive APIs
* use higher-ratio algorithms for cold storage
* compress logs and batch exports
* avoid double compression
* negotiate compression by client capability
* profile CPU cost
* separate hot path from archival path
...
* using expensive compression in a CPU-bound service
* forgetting mobile or edge client cost
* compressing encrypted or already-compressed data
* measuring bandwidth savings but not latency impact
...
> For **Compression vs Compute Trade-offs**, I would start by defining the target metric and measuring the current system with traces, metrics, and profiling.
>
...
> I would approach **Compression vs Compute Trade-offs** by measuring first, decomposing the critical path, and identifying the real bottleneck.
