---
title: 08 Evaluation System for LLM
level: interview
layout: post
categories:
...
note: e
feature_text: |
  ## From Traditional SD to LLM System Design - 08 Evaluation System for LLM
   Post by ailswan June. 07, 2026
feature_image: "https://picsum.photos/2560/600?image=1007"
---
...
This topic is about upgrading a **traditional system design** answer into an **LLM system design** answer for **Evaluation System for LLM**.

**Traditional SD baseline:**

A traditional evaluation system uses unit tests, integration tests, SLOs, dashboards, and production metrics.

**LLM system upgrade:**

LLM evaluation adds golden datasets, rubric grading, pairwise comparison, model-graded evals, human labels, prompt regression, retrieval eval, safety eval, and online feedback loops.
...
> For Evaluation System for LLM, I would start from the traditional system design baseline, then explain what changes when the core capability is powered by an LLM.
...
Production traces are sampled
  ↓
Examples are cleaned and labeled
  ↓
Rubrics are versioned
  ↓
Candidate prompt/model runs
  ↓
Model grader and human labels score outputs
  ↓
Regression gate compares baseline
  ↓
Canary rollout starts
  ↓
Online feedback is monitored
  ↓
...
> For Evaluation System for LLM, I would design a layered architecture.
...
For **Evaluation System for LLM**, I would specifically discuss:
...
A Staff-level LLM eval design treats quality as an engineering system. It version-controls prompts, models, datasets, rubrics, and rollout gates so quality changes are measurable and reversible.

**Memorize this answer:**

> For Evaluation System for LLM, I would not replace traditional system design with an LLM call.
...
A: A Staff-level LLM eval design treats quality as an engineering system. It version-controls prompts, models, datasets, rubrics, and rollout gates so quality changes are measurable and reversible.
...
>For Evaluation System for LLM, I would start with the traditional system design baseline and then upgrade it with LLM-specific components. The traditional baseline gives us API gateway, auth, stateless services, storage, cache, queues, scaling, and reliability. The LLM upgrade adds prompt/context construction, model routing, retrieval or memory, tool execution, safety checks, evaluation, token cost, and model observability.

### Memorization Paragraph 2

>The key design principle for Evaluation System for LLM is that the model should not be treated as the whole system. The model is one component behind an orchestration layer. The orchestration layer owns context, permissions, model choice, tool access, validation, cost control, and traces.

### Memorization Paragraph 3

>Compared with traditional SD, Evaluation System for LLM introduces probabilistic quality. That means I need both system metrics and quality metrics: latency, error rate, and cost on one side; groundedness, safety, user correction rate, and task success on the other side.
...
讨论 **Evaluation System for LLM** 时，我会按这个结构回答：
...
> 对于 Evaluation System for LLM，我会先从传统系统设计开始，而不是直接说调用 LLM。
...
在 **Evaluation System for LLM** 中具体要讲：
...
> 讲 Evaluation System for LLM 时，我会明确区分传统系统部分和 LLM 特有部分。
...
>Evaluation System for LLM 的回答不能停留在传统 SD，也不能只说调用 LLM。我的回答方式是先建立传统系统设计 baseline，然后增加 LLM orchestration layer。传统部分保证服务稳定，LLM 部分负责智能能力，系统边界负责安全、成本、评估和可观测性。

### 背诵段落 2

>传统系统设计主要关注 API、服务、数据库、缓存、队列和扩展。升级到 Evaluation System for LLM 后，还要关注 prompt version、model version、context window、retrieval、memory、tool calling、safety policy、eval pipeline 和 token cost。
...
>我会把 Evaluation System for LLM 拆成 online path 和 offline path。online path 负责用户请求、latency、safety 和 fallback；offline path 负责 index、eval、prompt test、cost analysis、quality regression 和 rollout。

### 背诵段落 5

>如果 Evaluation System for LLM 出现质量问题，我会通过 trace debug：用户输入是什么、context 是什么、prompt/model 版本是什么、retrieval 和 tool 结果是什么、safety decision 是什么、最终输出为什么通过。

---

## 19️⃣ 中文 Staff 总结

> Evaluation System for LLM 的 Staff 级答案，是从传统 SD 过渡到 LLM 系统设计。
> 传统 SD 提供稳定的分布式系统基础。
...
Evaluation System for LLM = Traditional SD baseline + LLM orchestration layer + safety/eval/cost/observability.
