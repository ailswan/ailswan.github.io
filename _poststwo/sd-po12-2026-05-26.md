---
title: "12 Reducing Cross-service Communication Cost"
level: interview
layout: post
categories:
- sd-po Performance & Optimization
feature_text: |
  ## Performance & Optimization - 12 Reducing Cross-service Communication Cost
   Post by ailswan May. 26, 2026
feature_image: "https://picsum.photos/2560/600?image=612"
---
...
When discussing **Reducing Cross-service Communication Cost**, I frame it as a performance problem with clear measurement, bottleneck isolation, and trade-off management.

1. identify fan-out and dependency graph
2. reduce unnecessary remote calls
3. prefer local computation or cached read models when safe
4. batch requests to the same dependency
5. choose efficient serialization and payload shape
6. co-locate services with tight latency coupling
7. use async events for non-critical communication
8. monitor cost in latency, bandwidth, CPU, and reliability
...
Cross-service communication is not just latency. It also adds serialization CPU, retries, failure modes, bandwidth cost, and operational coupling.
...
* calls per request
* fan-out width and depth
* RPC latency
* payload bytes
* serialization CPU
* retry amplification
* dependency error rate
* contract breakage incidents
...
* merge overly chatty APIs
* introduce read models for high-fanout reads
* use bulk endpoints
* cache dependency responses
* use event-driven propagation for non-critical updates
...
* over-microservicing a tightly coupled domain
* using synchronous calls for everything
* batching unrelated work and increasing tail latency
* creating circular service dependencies
* ignoring versioning and schema evolution
...
> For **Reducing Cross-service Communication Cost**, I would start by defining the target metric and measuring the current system with traces, metrics, and profiling.
>
...
> I would approach **Reducing Cross-service Communication Cost** by measuring first, decomposing the critical path, and identifying the real bottleneck.
