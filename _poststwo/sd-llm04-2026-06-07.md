...
note: e
feature_text: |
  ## From Traditional SD to LLM System Design - 04 Memory System Design
   Post by ailswan June. 07, 2026
feature_image: "https://picsum.photos/2560/600?image=963"
---
...
This topic is about upgrading a **traditional system design** answer into an **LLM system design** answer for **Memory System Design**.

**Traditional SD baseline:**

A traditional profile or preference system stores user data, session data, and history with access control and retention rules.

**LLM system upgrade:**

An LLM memory system adds short-term context, long-term extracted memories, semantic retrieval, recency/importance scoring, memory correction, and deletion governance.
...
> For Memory System Design, I would start from the traditional system design baseline, then explain what changes when the core capability is powered by an LLM.
...
Conversation happens
  ↓
Memory extractor identifies durable facts
  ↓
Classifier assigns memory type
  ↓
Scope and permission are attached
  ↓
Memory is embedded and stored
  ↓
Future request arrives
  ↓
Retriever finds relevant memories
  ↓
Freshness/privacy filter runs
  ↓
Context builder injects selected memory
```
...
> For Memory System Design, I would design a layered architecture.
...
For **Memory System Design**, I would specifically discuss:
...
A Staff-level memory design avoids transcript dumping. It extracts durable facts, scopes them, ranks them, supports deletion/correction, and only injects memory when it improves the current task.

**Memorize this answer:**

> For Memory System Design, I would not replace traditional system design with an LLM call.
...
A: A Staff-level memory design avoids transcript dumping. It extracts durable facts, scopes them, ranks them, supports deletion/correction, and only injects memory when it improves the current task.
...
>For Memory System Design, I would start with the traditional system design baseline and then upgrade it with LLM-specific components. The traditional baseline gives us API gateway, auth, stateless services, storage, cache, queues, scaling, and reliability. The LLM upgrade adds prompt/context construction, model routing, retrieval or memory, tool execution, safety checks, evaluation, token cost, and model observability.

### Memorization Paragraph 2

>The key design principle for Memory System Design is that the model should not be treated as the whole system. The model is one component behind an orchestration layer. The orchestration layer owns context, permissions, model choice, tool access, validation, cost control, and traces.

### Memorization Paragraph 3

>Compared with traditional SD, Memory System Design introduces probabilistic quality. That means I need both system metrics and quality metrics: latency, error rate, and cost on one side; groundedness, safety, user correction rate, and task success on the other side.
...
讨论 **Memory System Design** 时，我会按这个结构回答：
...
> 对于 Memory System Design，我会先从传统系统设计开始，而不是直接说调用 LLM。
...
> 讲 Memory System Design 时，我会明确区分传统系统部分和 LLM 特有部分。
...
>Memory System Design 的回答不能停留在传统 SD，也不能只说调用 LLM。我的回答方式是先建立传统系统设计 baseline，然后增加 LLM orchestration layer。传统部分保证服务稳定，LLM 部分负责智能能力，系统边界负责安全、成本、评估和可观测性。

### 背诵段落 2

>传统系统设计主要关注 API、服务、数据库、缓存、队列和扩展。升级到 Memory System Design 后，还要关注 prompt version、model version、context window、retrieval、memory、tool calling、safety policy、eval pipeline 和 token cost。
...
>我会把 Memory System Design 拆成 online path 和 offline path。online path 负责用户请求、latency、safety 和 fallback；offline path 负责 index、eval、prompt test、cost analysis、quality regression 和 rollout。

### 背诵段落 5

>如果 Memory System Design 出现质量问题，我会通过 trace debug：用户输入是什么、context 是什么、prompt/model 版本是什么、retrieval 和 tool 结果是什么、safety decision 是什么、最终输出为什么通过。

---

## 19️⃣ 中文 Staff 总结

> Memory System Design 的 Staff 级答案，是从传统 SD 过渡到 LLM 系统设计。
> 传统 SD 提供稳定的分布式系统基础。
...
Memory System Design = Traditional SD baseline + LLM orchestration layer + safety/eval/cost/observability.
